...
The data is read from source. The data is formatted for use in the preexisting pipeline. 
...
```{r prep-country-data}
  ...
  pop_data[pop_data=="United States"]<-"US"
  pop_data[pop_data=="United Kingdom"]<-"UK"

  country_data <- covid_19 %>%
...
ggplot(data = country_data,
  ...
  geom_label_repel(nudge_y = 0.01, direction = "y", hjust = 0.01) + 
  # turn off legend
  guides(color = FALSE) +
  # use pretty colors
  ...
  labs(
    ...
    y = "Cumulative number of deaths per million",
    title = "Cumulative number of deaths from COVID-19 per million people,for selected countries",
